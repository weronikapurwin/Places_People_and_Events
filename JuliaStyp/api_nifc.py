...
def get_all_letters():
    all_letters = search_letters(offset=0, limit=1050, return_composers=True, lang='en')
    api_letters = [letter_by_id(i, return_senders=True, return_addressees=True) for i in all_letters.index]
    api_letters = pd.DataFrame(api_letters)
    api_letters['composers'] = all_letters['composers']
    api_letters.to_csv(r'letters\all_letters.csv')


def prepare_letters():
    all_letters = pd.read_csv(r'letters\all_letters.csv', index_col='id')
    all_letters.replace(to_replace=[float('nan'), '[nan]', '[]'], value=None, inplace=True)
    prepared = prepared_letters(all_letters)
    prepared.to_csv(r'letters\letters_prepared.csv')
    xletters = xml_letters(all_letters)
    xletters.to_csv(r'letters\letters_xml.csv')
...
def read_places():
    ...
    with open(r'letters\places.csv', 'r', encoding='UTF-8') as file:
